...
class WCB_Category_Banner {
	...
	public function admin_scripts_and_styles() {
		...
		if ( $screen != null and $screen->id == "edit-product_cat" ) {
			...
			wp_enqueue_script( 'wcb_admin_script',
				  '/wp-content/themes/Art Beauty/woocommerce/woocommerce-category-banner/assets/js/wcb-admin.js',
			...
			wp_enqueue_style( 
				'wcb_admin_styles', 
				 '/wp-content/themes/Art Beauty/woocommerce/woocommerce-category-banner/assets/css/wcb-admin.css'
			);
		}
	}

	//по term_id получаем картинку категории и выводим во фронт энд
	public function getImage($id){
		//получаем id картинки
		$term_meta = get_option( "taxonomy_term_$id->term_id" );

		if(!empty($term_meta["banner_url_id"])){
 			$urlImage =  wp_get_attachment_url( $term_meta["banner_url_id"] );
	...
	*/	
	...
	#TODO написать свой метод где параметр будет ид категории по нему и будем получать картинку и выводить
